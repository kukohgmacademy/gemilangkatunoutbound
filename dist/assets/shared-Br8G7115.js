(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))l(a);new MutationObserver(a=>{for(const n of a)if(n.type==="childList")for(const s of n.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&l(s)}).observe(document,{childList:!0,subtree:!0});function o(a){const n={};return a.integrity&&(n.integrity=a.integrity),a.referrerPolicy&&(n.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?n.credentials="include":a.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function l(a){if(a.ep)return;a.ep=!0;const n=o(a);fetch(a.href,n)}})();function A(t="home"){const e=document.getElementById("navbar-mount");if(!e)return;const o=[{name:"Beranda",href:"/",key:"home"},{name:"Tentang Kami",href:"/about",key:"about"},{name:"Paket Outbound",href:"/paket",key:"paket"},{name:"Galeri",href:"/gallery",key:"gallery"},{name:"Blog",href:"/blog",key:"blog"},{name:"Kontak",href:"/contact",key:"contact"}],l=o.map(s=>{const c=t===s.key;return`
      <a href="${s.href}" class="nav-link ${c?"active-link":""}">
        ${s.name}
      </a>
    `}).join("");e.innerHTML=`
  ...
  <div>
    ...
    <p class="text-[9px] text-[#5A5A40] font-medium tracking-widest uppercase">Provider No.1 Batu & Malang</p>
        ...
        <div id="mobile-menu" class="hidden md:hidden pb-6 pt-2 border-t border-[#E0E0D6] space-y-2">
          ${o.map(s=>`
            <a href="${s.href}" class="block px-4 py-2.5 rounded-lg text-sm font-medium transition-all ${t===s.key?"bg-[#3A5A40]/10 text-[#3A5A40] font-bold":"text-[#5A5A40] hover:bg-[#F5F5F0] hover:text-[#3A5A40]"}">
              ${s.name}
            </a>
          `).join("")}
          <div class="pt-2 px-4">
  ...
  `;const a=document.getElementById("mobile-menu-btn"),n=document.getElementById("mobile-menu");a&&n&&a.addEventListener("click",()=>{n.classList.toggle("hidden")})}function k(){const t=document.getElementById("footer-mount");t&&(t.innerHTML=`
  ...
  `)}function g(){let t=document.getElementById("floating-wa");t||(t=document.createElement("div"),t.id="floating-wa",t.style.cssText="position: fixed; bottom: 24px; right: 24px; z-index: 9999; display: flex; align-items: center; pointer-events: auto;",t.className="group cursor-pointer",t.innerHTML=`
      <!-- Tooltip label -->
    ...
    `,document.body.appendChild(t));let e=document.getElementById("back-to-top-btn");e||(e=document.createElement("button"),e.id="back-to-top-btn",e.setAttribute("aria-label","Kembali ke Atas"),e.style.cssText="position: fixed; bottom: 92px; right: 28px; z-index: 9999; pointer-events: auto;",e.className="w-11 h-11 bg-[#3A5A40] hover:bg-[#2C4530] text-white rounded-full flex items-center justify-center shadow-xl transition-all duration-300 border border-white/30 transform hover:scale-110 active:scale-95 group",e.innerHTML=`
    ...
    `,e.addEventListener("click",()=>{window.scrollTo({top:0,behavior:"smooth"})}),document.body.appendChild(e))}function w(t={}){let e=document.getElementById("booking-modal");e||(e=document.createElement("div"),e.id="booking-modal",e.className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 hidden",e.innerHTML=`
    ...
    `,document.body.appendChild(e),document.getElementById("close-modal-btn").addEventListener("click",()=>{e.classList.add("hidden")}),document.getElementById("booking-modal-form").addEventListener("submit",async l=>{l.preventDefault();const a=document.getElementById("modal-package-name").value,n=document.getElementById("modal-user-name").value,s=document.getElementById("modal-user-phone").value,c=document.getElementById("modal-participants").value,b=document.getElementById("modal-event-date").value,h=document.getElementById("modal-notes").value,p=`Halo Admin GemilangKatunOutbond, saya ingin reservasi/tanya penawaran paket:%0A%0A*Nama/Instansi:* ${n}%0A*No WA:* ${s}%0A*Paket:* ${a}%0A*Jumlah Peserta:* ${c} Orang%0A*Tanggal Acara:* ${b||"Belum pasti"}%0A*Catatan:* ${h||"-"}`;window.open(`https://wa.me/6283115542771?text=${p}`,"_blank"),e.classList.add("hidden")})),document.getElementById("modal-package-id").value=t.id||"",document.getElementById("modal-package-name").value=t.title||"Paket Outbound GemilangKatunOutbond",e.classList.remove("hidden")}let i=[],r=0;function x(t=[]){var o,l,a;i=t;let e=document.getElementById("lightbox-modal");e||(e=document.createElement("div"),e.id="lightbox-modal",e.className="fixed inset-0 z-[99999] bg-black/85 backdrop-blur-md hidden items-center justify-center p-3 sm:p-5 select-none overflow-hidden",e.innerHTML=`
    ...
    `,document.body.appendChild(e),(o=document.getElementById("close-lightbox-btn"))==null||o.addEventListener("click",u),(l=document.getElementById("prev-lightbox-btn"))==null||l.addEventListener("click",()=>d(-1)),(a=document.getElementById("next-lightbox-btn"))==null||a.addEventListener("click",()=>d(1)),window.addEventListener("keydown",n=>{e.classList.contains("hidden")||(n.key==="ArrowLeft"?d(-1):n.key==="ArrowRight"?d(1):n.key==="Escape"&&u())}),e.addEventListener("click",n=>{n.target===e&&u()}))}function y(t=0,e=[]){if(e.length>0&&(i=e),x(i),!i||i.length===0)return;r=(t+i.length)%i.length,m();const o=document.getElementById("lightbox-modal");if(o){o.classList.remove("hidden"),o.classList.add("flex"),document.body.style.overflow="hidden";const l=document.getElementById("lightbox-card-wrapper");l&&(l.classList.remove("animate-modal-pop"),l.offsetWidth,l.classList.add("animate-modal-pop"))}}function d(t){!i||i.length===0||(r=(r+t+i.length)%i.length,m())}function m(){if(!i||i.length===0)return;const t=i[r],e=document.getElementById("lightbox-img"),o=document.getElementById("lightbox-caption"),l=document.getElementById("lightbox-desc"),a=document.getElementById("lightbox-counter"),n=document.getElementById("lightbox-category"),s=document.getElementById("lightbox-wa-btn");e&&(e.src=t.src,e.alt=t.title||"Foto Galeri GemilangKatunOutbond"),o&&(o.textContent=t.title||"Dokumentasi Outbound Malang Batu"),l&&(l.textContent=t.desc||`Dokumentasi asli kegiatan ${t.title||"outbound"} di kawasan Malang & Batu bersama provider resmi GemilangKatunOutbond.`),a&&(a.textContent=`Foto ${r+1} dari ${i.length}`),n&&(n.textContent=t.cat||t.category||"Galeri Outbound"),s&&(s.href=`https://wa.me/6283115542771?text=Halo%20Admin%20GemilangKatunOutbond,%20saya%20tertarik%20dengan%20kegiatan%20galeri:%20${encodeURIComponent(t.title||"")}`)}function u(){const t=document.getElementById("lightbox-modal");t&&(t.classList.add("hidden"),t.classList.remove("flex"),document.body.style.overflow="")}function f(){const t=document.querySelectorAll(".reveal");if(!("IntersectionObserver"in window)){t.forEach(o=>o.classList.add("active"));return}const e=new IntersectionObserver(o=>{o.forEach(l=>{l.isIntersecting&&(l.target.classList.add("active"),e.unobserve(l.target))})},{threshold:.01,rootMargin:"100px 0px 50px 0px"});t.forEach(o=>{e.observe(o)})}function v(){document.querySelectorAll('a[href$=".html"]').forEach(t=>{t.addEventListener("mouseenter",()=>{const e=t.getAttribute("href");if(e&&!document.querySelector(`link[rel="prefetch"][href="${e}"]`)){const o=document.createElement("link");o.rel="prefetch",o.href=e,document.head.appendChild(o)}},{once:!0})})}if(typeof window<"u"){const t=()=>{g(),f(),v()};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",t):t()}export{k as a,g as b,w as c,f as i,y as o,A as r};
